...
public class SwipeCardsLayout extends LinearLayout {
    ...
    private int yOffsetStep = 50; // view叠加垂直偏移量的步长
    private float scaleOffsetStep = 0.05f; // view叠加缩放的步长
    ...
    public SwipeCardsLayout(Context context, AttributeSet attrs, int defStyle) {
...
//        cardVisibleCount = mAdapter.getVisibleCardCount();
    ...
    public void setAdapter(BaseCardAdapter adapter) {
        ...
        cardVisibleCount = Math.min(cardVisibleCount, mCount);
        for (int i = mShowingIndex; i < mShowingIndex + cardVisibleCount; i++) {
            View childView = mAdapter.onCreateView(this);
            if (childView == null) {
                return;
            }
            if (i < mCount) {
...
//        child.setAlpha(alpha);
    ...
    private void adjustChildView(View child, int index) {
        if (index > -1 && index < cardVisibleCount) {
            int multiple = Math.min(index, cardVisibleCount - 2); // 最大个是MAX_VISIBLE，实际上重叠的时候只显示 maxVisibleCount-1 个，最顶端的view的multiple从0开始，则最大为MAX_VISIBLE-2
